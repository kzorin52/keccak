...
public abstract class Tests
{
    ...
    {
        using var g = new GZipStream(
            File.OpenRead(Path.Combine("..", "..", "..", "..", "..", "binaries",
                string.Format(fileFormat, type, useKeccakPadding ? "" : "SHA3-", hashLen))),
            CompressionMode.Decompress);
        using (var reader = new StreamReader(g))
        {
            return reader.ReadToEnd().Split('\r', '\n');
        }
    }

    protected static IEnumerable<(int bytes, byte[] data, string hash)> GetDataAndHash(TestType type, string[] lines)
    {
        switch (type)
        {
            case TestType.Short:
            case TestType.Long:
                for (var i = 0; i < lines.Length; i++)
                    ...
                    {
                        var len = int.Parse(lines[i].Split(' ')[2]);
                        if (len % 8 == 0) //only on whole byte ranges
                            yield return (len / 8, lines[i + 1].Split(' ')[2].FromHexString(),
                                lines[i + 2].Split(' ')[2]);
                        i += 2;
                    }
            ...
            case TestType.ExtremelyLong:
                for (var i = 0; i < lines.Length; i++)
                    if (!lines[i].StartsWith('#') && lines[i] != string.Empty)
                    {
                        yield return (int.Parse(lines[i].Split(' ')[2]),
                            Encoding.ASCII.GetBytes(lines[i + 1].Split(' ')[2]), lines[i + 2].Split(' ')[2]);
                        i += 2;
                    }
    ...
    {
        var lines = GetLines(type, hashLen, useKeccakPadding);
        foreach ((int, byte[], string) tup in GetDataAndHash(type, lines))
        {
            var sha3 = SHA3.Create("sha3" + hashLen);
            sha3.UseKeccakPadding = useKeccakPadding;
            var repeats = type == TestType.ExtremelyLong ? tup.Item1 : 1;
...
#endif
                for (var r = 0; r < repeats; r++) sha3.TransformBlock(tup.Item2, 0, tup.Item2.Length, tup.Item2, 0);
